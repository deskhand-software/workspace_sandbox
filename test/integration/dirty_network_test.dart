...
void main() {
  group('Network Isolation (Real World Check)', () {
    ...
    test('Should BLOCK network access (curl fails)', () async {
      ...
      try {
        final result =
            await ws.run('curl -I --connect-timeout 3 https://www.google.com');
        ...
        if (!networkFailed && Platform.isWindows) {
          print(
              'WARNING: Network isolation layer passed, but native isolation on Windows is WIP.');
          ...
          expect(networkFailed, isTrue,
              reason: 'Native layer should have blocked connection');
        }
      } catch (e) {
